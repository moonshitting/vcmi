	...
	controller(hero1, hero2)
{
	...
	for(int b=0; b < heroInst.size(); b++)
	{
		...
		fullbox[b] = std::make_shared<CPicture>(ENGINE->renderHandler().loadAnimation(AnimationPath::builtin("FullBodyImage"))->getImage(hero->getIconIndex(), 0), Rect(66, 0, 172, 514), -77+(77+800)*b, 0);
